...
class Hasher():
    def __init__(self):
        ...
        hashes['md4'] = lambda x: self._get_pycryptodome_md4_hash(x)
        ...
        hashes['google_hash'] = lambda x: self.google_hash(x)
    ...
    def google_hash(self, string):
        string_hash = hashlib.sha256(string.encode()).hexdigest()
        string_bytes = bytes.fromhex(string_hash)
        result = base64.urlsafe_b64encode(string_bytes).decode('utf-8').rstrip('=')
        return result

    def _get_pycryptodome_md4_hash(self, string):
        h = MD4.new()
...
def detect_for_debug():
    ...
    EMAIL = "hiddenwhispers123@gmail.com"
    ...
    URL = "https://www.google.com/pagead/1p-conversion/11483530895/?random=1624634596&cv=11&fst=1744192625641&bg=ffffff&guid=ON&async=1&gtm=45be5471v9191631410za200zb9176375788&gcs=G111&gcd=13t3t3t2t5l1&dma_cps=syphamo&dma=1&tag_exp=102509683~102788824~102803279~102813109~102887799~102926062~102975949~103016951~103021830~103027016~103047562~103051953&u_w=1280&u_h=1100&url=https%3A%2F%2Fwww.huggwaii.com%2Fcheckouts%2Fcn%2FZ2NwLWV1cm9wZS13ZXN0MTowMUpSQ1pHVkYzUzBKUlc3TU4wSFlUSDlEQQ%2Fprocessing&ref=https%3A%2F%2Fwww.huggwaii.com%2F&label=AisRCO3LlMUZEI-N4-Mq&hn=www.googleadservices.com&frm=0&tiba=Checkout%20-%20Huggwaii&currency_code=EUR&did=dYmNjMT%2CdN2ZkMj%2CdNTU0Yz%2CdNzYwYj&gdid=dYmNjMT.dN2ZkMj.dNTU0Yz&edid=dNzYwYj&gtm_ee=1&rdp=0&npa=0&pscdl=noapi&auid=442904395.1744192444&uaa=x64&uab=64&uafvl=Chromium%3B133.0.6943.16%7CNot(A%253ABrand%3B99.0.0.0&uamb=0&uam=&uap=Windows&uapv=10.0&uaw=0&ec_mode=c&fledge=1&capi=1&_tu=AAI&data=event%3Dadd_payment_info%3Btotal%3D52.26%3Bignore_referrer%3Dtrue&em=tv.1~em.2tqn62cna4VgePYsNZuk0KiTfXtsXD6bocVV3d8uCSY~co0.NL&fmt=3&ct_cookie_present=false&crd=CPLOsQIIscGxAgiwwbECCLHDsQIIisWxAgjCybECCJDJsQII08WxAgjrzLECCM_OsQII_86xAkondHJpZ2dlciwgZXZlbnQtc291cmNlPW5hdmlnYXRpb24tc291cmNlWgMKAQFiBAoCAgM&pscrd=&is_vtc=1&cid=CAQSKQCjtLzMucq8Z000eRER1uURGnsfZv786E-QMx1RyO6YGG_3oc3tf-JB&random=1497334920"
